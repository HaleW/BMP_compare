...
void picCmp(char inPicName_1[20], char inPicName_2[20], char outPicName[20])
{
    RGBQUAD **picRgb1, **picRgb2;    //定义图片RGB数组
    ...
    picRgb1 = (RGBQUAD **)malloc(sizeof(RGBQUAD*) * width);   //定义储存图片一的RGB数组
    for (i = 0; i < width; i ++)
        picRgb1[i] = (RGBQUAD *)malloc(sizeof(RGBQUAD) * height);

    picRgb2 = (RGBQUAD **)malloc(sizeof(RGBQUAD*) * width);   //定义储存图片二的RGB数组
    for (i = 0; i < width; i ++)
        picRgb2[i] = (RGBQUAD *)malloc(sizeof(RGBQUAD) * height);


    for (i = 0; i < width; i ++)
        for (j = 0; j < height; j ++)
        {
            fread(&picRgb1[i][j], sizeof(RGBQUAD), 1, inPic_1);    //读取图片一RGB
            fread(&picRgb2[i][j], sizeof(RGBQUAD), 1, inPic_2);    //读取图片二RGB
    ...
    for (i = 0; i < width; i ++)
        for (j = 0; j < height; j ++)
            fwrite(&picRgb1[i][j], sizeof(RGBQUAD), 1, outPic);    //写入RGB
